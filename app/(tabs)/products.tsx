...
export const PRODUCTS = [
  ...
  {
    id: 'callaway-elyte-triple-diamond',
    category: 'driver-head',
    brand: 'Callaway',
  ...
  {
    id: 'cleveland-rtx6-zipcore-56',
    category: 'wedge',
    brand: 'Cleveland',
    model: 'RTX6 ZipCore 56°',
    type: 'Sand Wedge',
    loft: '56°',
    bounce: '10°',
    grind: 'Mid',
  ...
  {
    id: 'cleveland-rtx6-zipcore-60',
    category: 'wedge',
    brand: 'Cleveland',
    model: 'RTX6 ZipCore 60°',
    type: 'Lob Wedge',
    loft: '60°',
    bounce: '8°',
    grind: 'Low',
  ...
  {
    id: 'tm-milled-grind-4-56',
    category: 'wedge',
    brand: 'TaylorMade',
    model: 'Milled Grind 4 56°',
    type: 'Sand Wedge',
    loft: '56°',
    bounce: '10°',
    grind: 'Standard',
  ...
  {
    id: 'ventus-tr-blue-6s',
    category: 'shaft-driver',
    brand: 'Fujikura',
    model: 'Ventus TR Blue 6S',
    type: '低旋宽容',
    weight: '63g',
    trajectory: '中低',
    kickPoint: '高',
  ...
  {
    id: 'tensei-1k-black-65s',
    category: 'shaft-driver',
    brand: 'Mitsubishi',
    model: 'Tensei 1K Black 65S',
    type: '低旋稳定',
    weight: '65g',
    trajectory: '中低',
    kickPoint: '高',
  ...
  {
    id: 'ust-lin-q-m40x-5s',
    category: 'shaft-driver',
    brand: 'UST Mamiya',
    model: 'LIN-Q M40X Red 5S',
    type: '弹射感强',
    weight: '55g',
    trajectory: '中高',
    kickPoint: '低',
  ...
  {
    id: 'ns-pro-modus3-105s',
    category: 'shaft-iron',
    brand: 'Nippon',
    model: 'Modus3 Tour 105 S',
    type: '中量均衡',
    weight: '105g',
    trajectory: '中',
    flex: 'S',
  ...
  {
    id: 'ns-pro-modus3-120x',
    category: 'shaft-iron',
    brand: 'Nippon',
    model: 'Modus3 Tour 120 X',
    type: '重量低旋',
    weight: '120g',
    trajectory: '低',
    flex: 'X',
  ...
  {
    id: 'gp-tour-velvet-midsize',
    category: 'grip',
    brand: 'Golf Pride',
    model: 'Tour Velvet Midsize',
    type: '全天候经典',
    size: 'Midsize',
    weight: '52g',
    texture: '橡胶',
  ...
  {
    id: 'superstroke-traxion-pistol',
    category: 'grip',
    brand: 'SuperStroke',
    model: 'Traxion Pistol GT',
    type: '推杆防扭',
    size: 'Oversize',
    weight: '75g',
    texture: '聚氨酯',
...
function localMatchScore(product: Product, profile: StoredUserProfile | null) {
  ...
  if ('speed' in product && product.speed) {
    const hit = String(product.speed).includes('+')
      ? speed >= Number(String(product.speed).replace('+mph', ''))
      : true;
    if (hit) {
      score += 20;
  ...
  if ('handicap' in product && product.handicap) {
    ...
    if (
      (h.includes('+') && handicap >= Number(h.replace('+', ''))) ||
      (h.includes('-') &&
        handicap >= Number(h.split('-')[0]) &&
        handicap <= Number(h.split('-')[1]))
    ) {
      score += 18;
...
export default function ProductsScreen() {
  ...
  const products = useMemo(() => {
    ...
    return PRODUCTS.filter((p) => {
      ...
      if (!q) return true;
      return `${p.brand} ${p.model} ${p.type} ${Object.values(p).join(' ')}`
        .toLowerCase()
        .includes(q);
    });
  }, [category, keyword]);
          ...
          return (
            <View key={product.id} style={s.card}>
              ...
              <View style={s.right}>
                ...
                <View style={s.tag}>
                  <Text style={s.tagText}>{product.type}</Text>
                </View>
                {paramRows.map(([k, v]) => (
        ...
        <View style={s.modalMask}>
          <View style={s.modalCard}>
            ...
            <Text style={s.modalSub}>
              {match.product.brand} {match.product.model}
            </Text>
            <Text style={s.modalScore}>匹配度评分：{match.score}</Text>
            {match.reasons.map((r) => (
              <Text key={r} style={s.modalReason}>
                - {r}
              </Text>
            ))}
