/**
 * 从 GolfPass 抓取球场记分卡并合并到 data/courses.json
 *
 * 用法: npm run scrape:golfpass
 * 依赖: npm install cheerio node-fetch@2
 */
...
function buildCourseRecord(pendingItem, par, yards, handicap, about, latLng, teeMeta) {
  ...
  return {
    id,
    nameEn,
    nameZh: pendingItem.nameZh || '',
    golfPassUrl: pendingItem.golfPassUrl,
    holes: 18,
    par,
    yards,
    strokeIndex: handicap,
    yardsTee: teeMeta.teeLabel,
    yardsTeeRow: teeMeta.teeRowLabel,
    totalPar: about.totalPar != null ? about.totalPar : totalParCalc,
    totalYards: about.totalYards != null ? about.totalYards : totalYardsCalc,
    courseRating: about.rating,
    slopeRating: about.slope,
    designer: about.designer,
    yearBuilt: about.yearBuilt,
    latitude: latLng.lat,
...
async function processOne(item, existingIds) {
  ...
  if (sumPar < 71 || sumPar > 73) {
    const err = new Error(`Par 总和 ${sumPar} 不在 71–73 范围`);
    err.debugHtml = html;
    throw err;
  }
...
function teeNote(teeLabel) {
  if (!teeLabel || teeLabel === 'Black') return '';
  return ` (using ${teeLabel} tees)`;
}
...
async function main() {
  ...
  for (let i = 0; i < data.pending.length; i++) {
    ...
    try {
      ...
      const ySum = record.yards.reduce((a, b) => a + b, 0);
      const note = teeNote(record.yardsTee);
      const line = `${record.nameEn} - ${record.totalPar}, ${ySum} yards, 18 holes${note}`;
      successRows.push({
        name: record.nameEn,
